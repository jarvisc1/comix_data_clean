...
countries <- c("at", "dk", "es", "fr", "it", "pl", "pt")
...
for (country_name in countries) {
  ...
  zip_file_name <- file.path(
                             paste("CoMix", toupper(country_name), Sys.Date(),
                                   sep = "_"))
  country_file_path <- file.path(country_name, "*")
  cmd <- paste0("7z a ", "zipped/", zip_file_name, " ",   country_file_path, " -p", pwc)
  cmds <- c(cmds, cmd)
