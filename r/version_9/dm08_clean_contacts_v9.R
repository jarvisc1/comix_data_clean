## Name: dm07_clean_contact_vars_v9.R
## Description: Clean the variables relating to the contact data.
## Input file: combined_7_v9.qs
## Functions:
## Output file: combined_8_v9.qs clean/contacts_v9.qs
...
input_name <-  paste0("combined_7_v9.qs")
input_data <-  file.path(dir_data_process, input_name)
output_name <- paste0("combined_8_v9.qs")
output_data <- file.path(dir_data_process, output_name)

## Save contact data
current_date <- Sys.Date()
output_cnts <- paste0("contacts_v9.qs")
...
vars <- c("country", "part_id", "panel", "wave", "row_id", "sample_type")
...
dt[!is.na(cnt_home), contact_flag := TRUE]
dt[is.na(cnt_home), contact_flag := FALSE]
...
dt[!is.na(age_min) & !is.na(cnt_home),
...
dt[, cnt_total_time := NA_character_]
...
dt[contact_flag == TRUE & cnt_home == 1, cnt_main_type := "Home"]
dt[contact_flag == TRUE & cnt_home == 0 & cnt_work == 1 , cnt_main_type := "Work"]
dt[contact_flag == TRUE & cnt_home == 0 & cnt_work == 0 & cnt_school == 1 & sample_type == "adult", cnt_main_type := "School"]

dt[contact_flag == TRUE & cnt_home == 0 & cnt_work == 0 & cnt_school == 0, cnt_other := 1]
