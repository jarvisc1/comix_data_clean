...
multi <- grep("duration|precaution", multi, value = TRUE, invert= TRUE)
...
dt_long <- dt_long[variable %like% "child" | 
                     !is.na(value)  & value > 0]
...
dt_prec <- dt_prec[!is.na(multiple_contacts_other_precautions) ]
dt_prec <- dt_prec[!is.na(multiple_contacts_work_precautions) ]
dt_prec <- dt_prec[!is.na(multiple_contacts_school_precautions) ]
...
dt_cnts[is.na(cnt_prec),cnt_prec := fifelse(cnt_work == "Yes",
                                            multiple_contacts_work_precautions, NA_character_)]
